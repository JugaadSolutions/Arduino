/*--------------------------------------------------------------------------------------
 ...
 Editted by: Trevor van Bremen - October 2013
 - Completely eliminated any reliance on SPI
 - Replaced every digitalWrite () call with a direct port access
 The direct port access now makes this library TOTALLY dependent on an Arduino Mega board
 although it would be easy to put them back.
...
void DMD::scanDisplayBySPI()
{
        ...
        for (int i=0;i<rowsize;i++) {
            sendByte(bDMDScreenRAM[offset+i+row3]);
            sendByte(bDMDScreenRAM[offset+i+row2]);
            sendByte(bDMDScreenRAM[offset+i+row1]);
            sendByte(bDMDScreenRAM[offset+i]);
        }
