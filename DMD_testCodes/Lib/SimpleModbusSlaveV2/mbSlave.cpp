...
void MBSLAVE:: modbus_configure(HardwareSerial *SerialPort,
											unsigned int baud,
...
void MBSLAVE:: modbus_update_comms(unsigned int baud, unsigned char byteFormat, unsigned char _slaveID)
{
...
boolean MBSLAVE:: modbus_update(unsigned char *count, unsigned char *_functionCode )
{
    ...
    if (buffer > 7) 
      ...
      if (id == slaveID || broadcastFlag) // if the recieved ID matches the slaveID or broadcasting id (0), continue
      {
        ...
        if (calculateCRC(buffer - 2) == crc) // if the calculated crc matches the recieved crc continue
        {
				  ...
				  *count = no_of_registers * 2;   //byte count;
				  unsigned char index;
				  ...
				  if (!broadcastFlag && (function == 3))
				  {
					  if (startingAddress < holdingRegsSize) // check exception 2 ILLEGAL DATA ADDRESS
					  {
						  if (maxData <= holdingRegsSize) // check exception 3 ILLEGAL DATA VALUE
						  {
							...
							*_functionCode = function;
														
							f3StartingAdd = startingAddress;
							dataReceived = true;
							return dataReceived;
							...
							callback( frame+address, maxData, startingAddress );
...
void MBSLAVE:: exceptionResponse(unsigned char exception)
{
...
unsigned int MBSLAVE:: calculateCRC(unsigned char bufferSize) 
{
...
void MBSLAVE:: sendPacket(unsigned char bufferSize)
{
...
void MBSLAVE:: sendResponse(unsigned short *buffer, unsigned char _dataCount, unsigned char function )
{
	// ID, function, noOfBytes, (dataLo + dataHi)*number of registers,
	//  crcLo, crcHi
	unsigned char responseFrameSize = 5 + _dataCount; 
	unsigned char index;
	unsigned char address;
	unsigned int crc16;
	unsigned int maxData = f3StartingAdd + _dataCount/2;   //maxdata = startingAddress+no_of_registers
	...
	Serial.write(maxData);
	Serial.write("\n");
	Serial.println("MB slave response");
		
	for (index = f3StartingAdd; index < maxData; index++)
	{
		  temp = buffer[index];
		  frame[address] = temp >> 8; // split the register into 2 bytes
